...
from utils.tools import get_video_metadata
...
class BaseDataset():
    ...
    def load_pain_dataset(dataset_path, label_path_train, label_path_val, batch_size, BICUBIC, resolution, phase, seq_len=-1, frame_stride=4):
        try: 
            ...
            [
                # transforms.CenterCrop(100),
                transforms.Resize(resolution, interpolation=BICUBIC),   # Biovid=100,100 , UNBC= 96,96
            ...
            temporal_transform = transforms.Compose([
                TemporalDownSample(factor=frame_stride),       # keep every 2nd frame
            ...
            if seq_len != -1:
                ...
                # dataset_train = PainVideoDatasets(dataset_path, label_path_train, transform=transform_dict[phase], seq_len=seq_len) 
            else:
            ...
            if label_path_val is not None:
                if seq_len != -1:
                    if "BAH" in dataset_path :
                        ...
                        dataset_val = TemporalPainDataset(dataset_path, label_path_val, transform=None, seq_len=seq_len, temporal_transform=temporal_transform, fps=fps, audio_sr=16000) 
                    # dataset_val = PainSeqDatasets(dataset_path, label_path_train, transform=None, seq_len=seq_len)
                    # dataset_val = PainVideoDatasets(dataset_path, label_path_train, transform=None, seq_len=seq_len)
                else:
                    dataset_val = PainDatasets(dataset_path, label_path_val)
                
                shuffle = True
            elif "FERV39k" in dataset_path:
                dataset_val = dataset_train
            elif phase != 'tar': # for biovid
    ...
    def load_bah_dataset(dataset_img_path, datset_img_aug_val_path, label_path_train, label_path_val, 
        ...
        try: 
            ...
            # if is_stress:
            #     dataset_train = StressDatasets(dataset_img_path, label_path_train, transform=transform_dict[phase], only_bio_mod=only_bio)
            # else:
            ...
            if label_path_val is not None:
                ...
                # if is_stress else PainImgBioDatasets(dataset_img_path, label_path_val, only_bio_mod=only_bio)
                shuffle = True
            ...
            train_loader = DataLoader(  dataset_train,
                                        ...
                                        pin_memory=True,
                                        shuffle=shuffle,
                                        # collate_fn=BaseDataset.collate_fn if is_stress else None,
                                        sampler=SubsetRandomSampler(train_idx) if len(train_idx) != 0 else None
                                    )
